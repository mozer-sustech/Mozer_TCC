...
file1 = open('MD_TRAJECTORY.xyz-pos-1.xyz', 'r')
output = open('bond-length.txt', 'w')
linenum_1 = int(ele_1) + 2  # 第一个原子所在行数
linenum_2 = int(ele_2) + 2  # 第二个原子所在行数
total_atom = int(linecache.getline('MD_TRAJECTORY.xyz-pos-1.xyz', 1))  # 总原子数

def f(x, y):
    xyz_1 = linecache.getline('MD_TRAJECTORY.xyz-pos-1.xyz', x).strip('\n').split()  # 读取目标行
    xyz_1.pop(0)  # 去掉每行元素项
    f_1 = []
    for i in xyz_1:
        f_1.append(float(i)) #把xyz三个数值写入f_1
    xyz_2 = linecache.getline('MD_TRAJECTORY.xyz-pos-1.xyz', y).strip('\n').split()
    xyz_2.pop(0)
    f_2 = []
    for i in xyz_2:
        f_2.append(float(i))#把xyz三个数值写入f_2
    a1 = np.array(f_1)
    a2 = np.array(f_2)
    bond = a1 - a2
...
while linecache.getline('MD_TRAJECTORY.xyz-pos-1.xyz', linenum_1) != '':
    print(linenum_1, linenum_2)
